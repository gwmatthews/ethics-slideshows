class:  middle, center

### *Social Contract Theory*

#### *ethics by agreement*

![:scale 50%, #222;](img/07/police-2-gimp-workshop.jpg)

George Matthews, Pennsylvania College of Technology

*2020*

.note[press "h" for help on keyboard controls]

---
### *The ties that bind*

--

- Our social lives depend on our willingness to cooperate -- to sometimes set aside our own interests for the sake of others.

--

- *Trust* is central to cooperation -- my willingness depends on your willingness and my trust that you will follow through.

--

.topcap[

Our next question:

]

--

.argument[

Is there a *rational basis* for trust, or must trust be based on something besides reason, like feelings of sympathy, shared history, or emotional identification?


]

---
layout:true

### *Traditional and modern societies*
---

--

.pull-left[
.center[

#### traditional

]
]

--

.pull-right[
.center[

#### modern

]
]

---

.pull-left[
.center[

#### traditional

]

.example[

Rooted in time, place, culture, established authority and inherited roles.

Conservative values: emphasis on stability and hierarchy.

]
]

.pull-right[
.center[

#### modern

]
]

---

.pull-left[
.center[

#### traditional

]

.example[

Rooted in time, place, culture, established authority and inherited roles.

Conservative values: emphasis on stability and hierarchy.

]
]

.pull-right[
.center[

#### modern

]

.example[

Less bound by established tradition and hierarchy, society is an arena in which roles and rules are established by competition between individuals.

Liberal values: emphasis on progress and change. 

]
]


--

<br>

.note[

Most real societies are somewhere in the middle of these two, with some traditional and some modern institutions, aspects and values.

]

---
layout: true

### *Hobbes and the Social Contract*

.left-column[

<br>

![:scale 80%, #222;](img/07/hobbes.jpg)

.center[
.note[1588-1679
]
]
]
---

--

.right-column[

Thomas Hobbes was the founder of modern political philosophy.

]

--

.right-column[

His 1651 book *Leviathan* asks:

]

--

.right-column[
.narrow-text[

If there were no religion, no customs and no laws, how and why would a individuals form a society, establish laws and what would they look like?

]
]

---

.right-column[

If there were no rules, and we lived in a "State of Nature," we would be free to do as we please, thus leading to "war of all against all."

]

--

.right-column[
.narrow-text[

*"No arts; no letters; no society; and which is worst of all, continual fear and danger of violent death; and the life of man solitary, poor, nasty, brutish, and short."*

]
]

---

.right-column[

Thus it makes sense that we would agree to establish rules limiting individual liberty in exchange for the security.

]

--

.right-column[

We would create an "artificial person" and give it power to write and enforce laws regulating social interaction.

]

--

.right-column[

This is the Social Contract that establishes moral and social rules.

]

---

.right-column[
.topcap[

What about trust?

]
]

--

.right-column[

Outside of traditional society, trust is hard to come by, due to our competitive natures. 

]

--

.right-column[

For Hobbes the function of the social contract is to establish an absolute ruler and avoid the issue of trust. 

]

--

.right-column[

But then we might wonder whether we can trust an absolute ruler to rule according to principles of justice...

]


---
layout: false

### *An argument for Social Contract Theory*

--

Hobbes' reflections suggest an argument for moral rules.

--

.topcap[

Morals by agreement

]

--

.argument[

A world in which there were no rules regulating our behavior would be unbearable.

Thus we all have an interest in establishing and abiding by basic rules against lying, stealing, murder, not keep promises, etc.

***

Thus we create and follow such rules.

]

---

.right-column[

What about trust?

]

--

.right-column[

Hobbes thinks we can't trust each other in the state of nature and so have to set up formal rules of social life in the Social Contract.

]

--

.right-column[

It is only by giving up liberty to an *absolute monarch* that we can solve the problem of not being able to trust each other -- under the Social Contract we have no choice but to follow the rules.

]

--

.right-column[

Yet why should we *trust the monarch* him or herself to follow the rules?

]

---
layout: true

### *The prisoner's dilemma*

---



---
layout: true

### *Locke's classical liberalism*

.left-column[

<br>

![:scale 80%, #222;](img/07/locke.jpg)

.center[
.note[1632-1704
]
]
]

---




---
layout: false

class: center credits

![:scale 50%, #222;](img/07/police-1-mabel-amber.jpg) 

#### Credits

*Built with:*

[Rstudio](https://rstudio.com/products/rstudio/) 

[xarignan](https://github.com/yihui/xaringan) html presentation framework 

*Photos by:*

 and  at Pixabay

**For more slideshows visit:**

[ethics slideshows home page](https://gwmatthews.github.io/ethics-slideshows/index.html)

[download this presentation](./pdf/07-contract.pdf) or [print it](./pdf/07-contract-print.pdf)

[editorial suggestions and comments](https://github.com/gwmatthews/ethics-slideshows/issues): requires a (free) GitHub account.
